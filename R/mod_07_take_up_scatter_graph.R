    ...
    la_imd_count <- lowIncomeSchemeScrollytellR::imd_decile_df %>% 
      ...
      dplyr::mutate(
        data = purrr::map(data, highcharter::mutate_mapping, 
                          highcharter::hcaes(x = INDEX_OF_MULT_DEPRIV_DECILE,
                                             y = IMD_DECILE_P), drop = TRUE),
    ...
    highcharter::highchart(type = "map") %>% 
      ...
      highcharter::hc_tooltip(
        ...
        pointFormatter = highcharter::tooltip_chart(
          accesor = "ttdata",
          hc_opts = list(
            title = list(text = "point.PCD_LAD_NAME", 
                         size = 7),
            chart = list(type = 'column'),
            xAxis = list(title = list(text = "Deprivation"), # doesn't show and i dont know why!
                         min = 1,
                         max = 10,
                         type = "category",
                         labels = list(step = 1)),
            yAxis = list(title = list(text = '% in deprivation',
                                      align = 'high'),
                         type = "category",
