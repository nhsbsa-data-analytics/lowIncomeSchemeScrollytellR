...
mod_08_spotlight_students_server <- function(input, output, session) {
  ...
  output$plot_successful_student_individuals_by_region <- highcharter::renderHighchart({
    ...
    plot_df <- lowincomeschemeucd::student_population_df %>%
      dplyr::filter(ACADEMIC_YEAR != "2014/15") %>% 
      dplyr::inner_join(lowincomeschemeucd::successful_student_individuals_by_region_df) %>%
      dplyr::mutate(
  ...
  output$plot_student_applications_per_month <- highcharter::renderHighchart({
    ...
    plot_overall_df <- lowincomeschemeucd::applications_df %>%
    ...
    plot_benefit_df <- lowincomeschemeucd::applications_df %>%
      dplyr::filter(
